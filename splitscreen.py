...
class SplitScreen:
    ...
    def initialize(self, stdscr):
        ...
        self.right_win = curses.newwin(
            self.height - 3, self.col_width, 0, self.col_width
        )
    ...
    def trim_space_from_tails(self):
        if len(self.left_lines) >= 2 and len(self.right_lines) >= 2:
            if (
                "" == self.left_lines[-1].strip()
                and "" == self.left_lines[-2].strip()
                and "" == self.right_lines[-1].strip()
                and "" == self.right_lines[-2].strip()
            ):
                self.left_lines.pop()
    ...
    def output_text(self, left_msg, right_msg):
        ...
        if (
            left_rets <= 1
            and right_rets <= 1
            and left_msg.startswith("> ")
            and right_msg.startswith("> ")
        ):
